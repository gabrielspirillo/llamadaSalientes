  ...
  if (!tenant && !selfRegister && !authenticated) {
    return (
      <div className="mx-auto flex max-w-md flex-col items-center px-4 py-24 text-center">
        <h1 className="text-[23px] font-extrabold tracking-tight text-zinc-900">
          Enlace incompleto
        </h1>
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-2xl flex-col px-4 py-8 sm:px-6 sm:py-12">
      ...
      <header className="mb-6">
          ...
          {STEPS.map((st) => {
            ...
            return (
              <li key={st.n} className="flex flex-1 items-center gap-1.5">
                <span
                  title={st.title}
                  className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[12px] font-bold transition-all duration-500 ${
                    done
        ...
        <div>
          <p className="text-[12px] font-bold uppercase tracking-[0.14em] text-brand-500">
            Paso {step} de {STEPS.length} · {progress}%
          </p>
          <h1 className="mt-1 text-[26px] font-extrabold tracking-tight text-zinc-900 sm:text-[31px]">
            {current.title}
          </h1>
        </div>
      </header>
      ...
      {submitError && (
        <p className="mt-4 animate-fade-up rounded-2xl border border-rose-100 bg-rose-50/80 px-4 py-3 text-[14px] text-rose-700">
          {submitError}
        </p>
      )}
