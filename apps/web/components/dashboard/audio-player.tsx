...
export function AudioPlayer({ callId }: Props) {
  ...
  async function load() {
    ...
    setErrorMsg(null);
    try {
      // Pre-flight: HEAD para detectar 425 / 404 sin descargar el blob.
      const head = await fetch(`/api/calls/${callId}/recording`, { method: 'HEAD' });
      if (head.status === 425) {
        setLoadState('too-early');
        setErrorMsg('La grabación todavía no está lista');
        return;
      }
      if (head.status === 404) {
        setLoadState('error');
      ...
      // Asignamos src — los events loadedmetadata/canplay van a setear 'ready'.
      // Cache-buster para que reintentar realmente recargue.
      if (audioRef.current) {
        audioRef.current.src = `/api/calls/${callId}/recording?t=${Date.now()}`;
        ...
        <div className="flex items-start gap-3 rounded-lg bg-amber-50 border border-amber-200 p-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              {errorMsg ?? 'Retell la procesa entre 30-90 segundos después de colgar.'}
            </p>
      ...
      {loadState === 'ready' && (
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={togglePlay}
            className="h-12 w-12 shrink-0 inline-flex items-center justify-center rounded-full bg-[linear-gradient(120deg,#37766a,#5fa896)] text-white shadow-[0_10px_26px_-10px_rgba(55,118,106,0.85)] transition-all duration-300 hover:scale-105 active:scale-95"
          >
