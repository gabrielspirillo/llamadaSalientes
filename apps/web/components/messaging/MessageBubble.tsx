  ...
  if (message.kind === 'SYSTEM' && !deleted) {
    return (
      <li className="flex justify-center px-4 py-1.5">
        <span className="inline-flex max-w-[80%] items-center gap-1.5 rounded-full bg-white/70 px-3 py-1 text-[11.5px] text-zinc-500 ring-1 ring-[--color-border-subtle]">
      ...
      <div className="w-8 shrink-0 pt-0.5">
        {grouped ? (
          <time
            ...
            className="hidden select-none text-[10px] font-medium leading-5 text-zinc-400 group-hover:block"
          >
      ...
      <div className="min-w-0 flex-1">
        {!grouped && (
          <div className="mb-0.5 flex flex-wrap items-baseline gap-2">
            <span className="text-[13px] font-bold tracking-tight text-zinc-900">{senderName}</span>
            {isSystem && (
              <span className="rounded-md bg-brand-100 px-1.5 py-px text-[9.5px] font-bold uppercase tracking-[0.1em] text-brand-700">
        ...
        {deleted ? (
          <p className="text-[13px] italic text-zinc-400">Mensaje eliminado</p>
        ) : editing ? (
          <div className="animate-zoom-in">
            <textarea
              ...
              className="w-full rounded-[14px] border border-brand-200 bg-white p-3 text-[13.5px] leading-relaxed text-zinc-900 focus-visible:border-brand-400 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-500/12"
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[10.5px] text-zinc-400">Esc para cancelar</span>
            </div>
          </div>
        ) : (
          <div
          ...
          >
            {isDecision && (
              <p className="mb-1 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-[0.12em] text-brand-700">
        ...
        {message.failed && (
          <div className="mt-1 flex items-center gap-2 text-[11.5px] text-rose-600">
      ...
      {message.reactions.map((r) => {
        ...
        return (
          <button
            ...
            className={cn(
              'press inline-flex animate-pop items-center gap-1 rounded-full px-2 py-0.5 text-[11.5px] font-semibold transition-all duration-200',
              mine
      ...
      {files.map((a) => (
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-[12.5px] font-semibold text-zinc-800">
              {a.name}
            </span>
            <span className="block text-[10.5px] text-zinc-400">{formatBytes(a.size)}</span>
          </span>
