  ...
  return (
    ...
    >
      ...
      <header className="flex shrink-0 items-center gap-3 border-b border-[--color-border-subtle] px-3 py-3 sm:px-4">
        ...
        {!connected && (
          <span className="hidden rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-amber-700 sm:inline">
            Reconectando
          </span>
        )}
      ...
      {showJump && (
        <button
          ...
          className="press absolute bottom-28 left-1/2 z-20 inline-flex -translate-x-1/2 animate-fade-up items-center gap-1.5 rounded-full bg-[linear-gradient(120deg,#7139e8,#a855f7)] px-3.5 py-1.5 text-[12px] font-semibold text-white shadow-[0_10px_30px_-8px_rgba(113,57,232,0.75)]"
        >
          Nuevos mensajes
      ...
      {dragging && (
        <div className="spotlight pointer-events-none absolute inset-2 z-30 flex animate-fade-in items-center justify-center rounded-[20px] border-2 border-dashed border-brand-400 bg-white/80 backdrop-blur-sm">
          <div className="text-center">
            ...
            <p className="text-[15px] font-semibold text-zinc-800">Soltá para adjuntar</p>
            <p className="mt-1 text-[12.5px] text-zinc-500">Se sube y se manda con tu mensaje</p>
          </div>
        </div>
      )}
    </Card>
