...
export interface ToneMeta {
  /** Tono de `Card` del sistema Aurora. */
...
export function toneMeta(tone: ImTone | null | undefined): ToneMeta {
  return TONE_META[tone ?? 'grape'] ?? TONE_META.grape;
}

/** Barra de acento del ítem activo — la misma del `NavLink` del sidebar. */
export const ACTIVE_BAR = 'bg-[linear-gradient(180deg,#7139e8,#ec4899)]';
  ...
  return (
    <span
      className={cn(
        ...
        strong ? 'bg-brand-600 text-white' : 'bg-violet-100/90 text-violet-700',
      )}
    >
      @{label}
    </span>
