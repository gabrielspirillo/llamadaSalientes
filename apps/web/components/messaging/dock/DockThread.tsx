  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      >
            ...
            return (
              ...
              >
                ...
                <div className={cn('min-w-0 max-w-[78%]', mine && 'text-right')}>
                  {!grouped && !mine && (
                    <p className="mb-0.5 truncate text-[11px] font-semibold text-zinc-500">
                      {m.senderName ?? 'Sistema'}
                    </p>
                  )}
                  <div
                    className={cn(
                      'inline-block rounded-[16px] px-3 py-2 text-left text-[13px] leading-snug transition-opacity duration-200',
                      mine
        ...
        {peers.length > 0 && (
          <p className="px-1 text-[11px] italic text-zinc-400">
            {peers.length === 1
      ...
      <div className="flex shrink-0 items-center gap-2 border-t border-white/60 bg-white/70 px-3 py-2.5">
        <input
          ...
          aria-label="Mensaje"
          className="h-10 min-w-0 flex-1 rounded-full border border-[--color-border] bg-white px-4 text-[13px] outline-none transition-colors placeholder:text-zinc-400 focus:border-brand-300"
        />
        <button
          type="button"
          onClick={() => void onSend()}
          ...
          aria-label="Enviar"
          className="sheen press inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[linear-gradient(120deg,#7139e8,#a855f7)] text-white shadow-[0_8px_20px_-10px_rgba(113,57,232,0.9)] transition-all disabled:pointer-events-none disabled:opacity-40"
        >
