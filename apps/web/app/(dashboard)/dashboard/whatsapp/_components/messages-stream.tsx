    ...
    >
      {empty ? (
        <div className="flex h-full flex-col items-center justify-center gap-2 text-center">
          ...
          <p className="text-[14px] text-zinc-500">Sin mensajes aún.</p>
        </div>
      ) : (
...
function MessageBubble({
  ...
  return (
    <li
      className={`max-w-[85%] animate-fade-up px-3.5 py-2.5 text-[14px] leading-relaxed sm:max-w-[70%] ${containerCls}`}
    >
      {isInternal && (
        <div className="mb-1 text-[11px] font-semibold uppercase text-amber-700">
          Nota interna{authorEmail ? ` · ${authorEmail}` : ''}
        </div>
      )}
      {!isInternal && isOutbound && authorEmail && (
        <div className="mb-0.5 text-[11px] font-medium text-emerald-100">{authorEmail}</div>
      )}
